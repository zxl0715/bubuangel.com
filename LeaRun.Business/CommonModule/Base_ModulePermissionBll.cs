...
namespace LeaRun.Business
{
    ...
    public class Base_ModulePermissionBll : RepositoryFactory<Base_ModulePermission>
    {
        ...
        /// <returns></returns>
        public bool ActionAuthorize(string Action, string ObjectId, string ModuleId)
        {
            List<Base_Module> ListData = new List<Base_Module>();
            object ActionAuthorize_List = DataCache.Get("ActionAuthorize_List");
            if (ActionAuthorize_List == null)
            {
                ...
                strSql.Append(" UNION ");
                strSql.Append(@"SELECT  m.ModuleId,m.FullName , m.Location FROM    Base_Module m LEFT JOIN Base_ModulePermission mp ON mp.ModuleId = m.ModuleId  AND mp.ObjectId IN ('" + ManageProvider.Provider.Current().ObjectId.Replace(",", "','") + "')");
                ListData = DataFactory.Database().FindListBySql<Base_Module>(strSql.ToString());
                DataCache.Insert("ActionAuthorize_List", ListData);
            }
            else
            {
